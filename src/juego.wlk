...
object nombreDelJuego {
	...
	method aparecerObjetos(){
		game.onTick(3000, "aparecer aleatoriamente", { self.crearYMoverGrande()})	
		game.onTick(2000, "aparecer aleatoriamente", { self.crearYMoverMediano()})
		game.onTick(1000, "aparecer aleatoriamente", { self.crearYMoverChico()})
	}
	...
	method crearYMoverMediano(){
		...
		miObjeto.aparecer()
		game.onTick(280, "moverse", {miObjeto.moverse(miObjeto.position().left(1))})
	}
	
	method crearYMoverChico(){
		const miObjeto = new ObjetoChico(posicion = game.at(20, 10))
		miObjeto.aparecer()
		game.onTick(260, "moverse", {miObjeto.moverse(miObjeto.position().left(1))})
	}
